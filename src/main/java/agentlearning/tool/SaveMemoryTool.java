...
public class SaveMemoryTool implements Tool {
    ...
    public SaveMemoryTool(MemoryManager memory, Runnable afterSave) {
        this.memory = memory;
    ...
    @Override public String execute(String argumentsJson) {
        try {
            ...
            if (fact.isBlank()) return "保存失败：fact 不能为空";
            memory.storeFact(fact.trim());                    // ← 用 storeFact
            if (afterSave != null) afterSave.run();
            return "已保存到长期记忆: " + fact.trim();
        } catch (Exception e) {
