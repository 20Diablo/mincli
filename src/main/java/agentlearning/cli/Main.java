...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        String basePrompt = """
                ...
                检索策略：
                - 当用户用自然语言描述"找 / 搜索 / 定位"某个功能，且你不确定具体文件或符号名时，
                  优先调用 search_code 工具做语义检索，再基于检索结果回答。
                - 需要精确定位已知符号/字符串时，用 read_file / list_dir 直接读；
                  search_code 是"说不清具体在哪、只知道想干嘛"时的语义兜底。
                - 调用 search_code 前，代码库需要已建索引（由用户执行 /index）。
